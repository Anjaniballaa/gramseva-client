...
// Field Officer (GramSevak) screens
...
// ── BUILD TAB OPTIONS WITH THEME ──
const buildTabOptions = (activeColor, colors) => ({
...
function FarmerTabs() {
  ...
  const opts = buildTabOptions(colors.farmer, colors);
...
// ═══════════════════════════════════════
// RESIDENT (VILLAGER) TABS
// ═══════════════════════════════════════
function VillagerTabs() {
  const { colors } = useTheme();
  const opts = buildTabOptions(colors.resident, colors);

  return (
    <Tab.Navigator screenOptions={opts}>
      <Tab.Screen
        name="Home"
        component={VillagerHome}
        options={{ tabBarIcon: tabIcon('home'), tabBarLabel: 'Home' }}
      />
      <Tab.Screen
        name="Health"
...
// ═══════════════════════════════════════
// FIELD OFFICER (GRAMSEVAK) TABS
// ═══════════════════════════════════════
function GramSevakTabs() {
  const { colors } = useTheme();
  const opts = buildTabOptions(colors.fieldOfficer, colors);

  return (
    <Tab.Navigator screenOptions={opts}>
      <Tab.Screen
        name="Home"
        component={GramSevakHome}
        options={{ tabBarIcon: tabIcon('home'), tabBarLabel: 'Home' }}
      />
      <Tab.Screen
        name="Reports"
        component={VillageReportsScreen}
        options={{
...
function DoctorTabs() {
  const { colors } = useTheme();
  const opts = buildTabOptions(colors.doctor, colors);
...
export default function AppNavigator() {
  ...
  if (user?.role === 'farmer') {
    return (
      <Stack.Navigator screenOptions={stackOptions}>
        ...
        <Stack.Screen
          name="NearbyHealthCenters"
          component={NearbyHealthCentersScreen}
        />
      </Stack.Navigator>
    );
  }
  ...
  // But if it does, show farmer home as default
  return (
    <Stack.Navigator screenOptions={stackOptions}>
      <Stack.Screen name="Main" component={FarmerTabs} />
    </Stack.Navigator>
